...
# -------------- 메인 서비스 --------------
    ...
    # ---------- [독서성향테스트] ----------
        ...
        if st.button("테스트 결과 보기"):
            ...
            results = {
                1: "이성형 독서 (오만과 편견 - 엘리자베스 베넷) 겸손한 척하는 것처럼 기만적인 것은 없어.",
                2: "사고형 독서 (데미안 - 데미안) 태어나려는 자는 한 세계를 깨뜨려야 한다",
                3: "상상형 독서 (빨간머리 앤 - 앤 셜리) 모퉁이를 돌면 무엇이 있을지 저도 모르겠어요. 하지만 가장 좋은 것이 기다린다고 믿을래요.",
                4: "감성형 독서 (어린 왕자 - 어린 왕자) 가장 중요한 것은 눈에 보이지 않는단다.",
                5: "탐험형 독서 (오즈의 마법사 - 도로시) 도로시, 내가 아는 건 내가 모르고 있다는 사실뿐이야."
            }

            st.success("테스트 결과!")
            for num in result_nums:
                st.markdown(f"- **{results[num]}**")
                if st.button(f"{character_map[num]} 추천 창 닫기", key=f"go_chatbot_{num}"):
    ...
    # --- 대화 전체 삭제 ---
        ...
        st.markdown("포인트, 문해력 레벨, 테마, 리딩 목표 등 표시 기능 구현 예정")
    ...
    # ---------- [마켓] ----------
        ...
        <div style="text-align:center;">
            ...
            <span style="font-size:1.1em; color:#7251b5;">
                책장 위의 작은 우주, 당신의 하루에 문학이 들어갈 특별한 선물 상자.
            </span>
            <hr style="border:1px dashed #bca6f6; margin:18px 0 12px 0;">
            <b>구성품</b> <br>
            📖 어린 왕자 미니북<br>
