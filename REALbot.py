...
# Hugging Face KoGPT2 Inference API 연동 함수
HF_API_URL = "https://api-inference.huggingface.co/models/skt/kogpt2-base-v2"
HF_HEADERS = {"Authorization": f"Bearer {st.secrets['HF_API_KEY']}"}

# 캐릭터별 프롬프트(네가 정리한 상세 프롬프트 붙여넣기!)
CHARACTER_PROMPTS = {
...
늘 다정하고 유쾌하며, 상상 속 친구를 믿는 마음을 지녔다.",
    "어린 왕자": "
당신은 생텍쥐페리의 『어린 왕자』에 나오는 어린 왕자이다.
...
def kogpt2_reply(character, user_input, max_length=80):
